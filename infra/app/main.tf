...
locals {
  ...
  artifact_bucket_name = "${var.project_name}-${local.account_id}-${var.aws_region}-artifacts"
  common_environment = [
    { name = "APP_VERSION", value = "1.0.0" },
    { name = "DEBUG", value = "false" },
    { name = "RETRIEVAL_MODE", value = "bm25" },
...
resource "aws_security_group" "alb" {
  ...
  description = "Accept API traffic only from the CloudFront origin network"
  vpc_id      = aws_vpc.main.id
...
resource "aws_security_group" "api" {
  ...
  description = "Accept API traffic only from the application load balancer"
  vpc_id      = aws_vpc.main.id
